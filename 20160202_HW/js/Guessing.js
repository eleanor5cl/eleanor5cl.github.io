...
//亂數產生n位數，各個數字不重複
{
    ...
    for (var i = 1; i < len; i++) {
		ran_num[i] = parseInt(Math.random()*10).toString();
        while (true) {
...
/*			if (i != j && ran_num[i] == ran_num[j]) {
    ...
    document.forma.Text1.value = ran_num[0] + ran_num[1] + ran_num[2] + ran_num[3];
...
function calculate(f) {
    ...
    //var v = ""
	f.g_num.value = new Array(t1, t2, t3, t4);
    var v = f.g_num.value;
    for (var i = 0; i < len; i++) {
